...
def UseItemDirect (slot_index):
	"""Close menu first, then dispatch item use -- mirrors spell touch pattern."""
	FloatMenuWindow.Close ()
	pc = GemRB.GameGetFirstSelectedPC ()
	if not pc:
		return
	GemRB.GameSelectPC (pc, 1, SELECT_REPLACE | SELECT_QUIET)
	GemRB.UseItem (pc, slot_index + 21, 0)
...
def OpenFloatMenuWindow (x=0, y=0):
	...
	def OnClose():
		...
		if float_menu_mode == MENU_MODE_WEAPONS:
			UseWeapon()
...
def _GroupActionPress(callback):
	FloatMenuWindow.Close ()
	callback ()
...
def UpdateFloatMenuGroupAction (i):
	...
	Button.OnPress (lambda btn=None, cb=acts[i]: _GroupActionPress(cb))
	Button.SetState (IE_GUI_BUTTON_ENABLED)
...
def UpdateFloatMenuItem (pc, i, weapons):
	...
	if weapons and i==4:
		...
		Button.OnPress (lambda btn=None: _GroupActionPress(ActionsWindow.ActionDefendPressed))
		Button.SetState (IE_GUI_BUTTON_ENABLED)
		return
	...
	if slot_item and slot_item['Flags'] & IE_INV_ITEM_IDENTIFIED:
		...
		if weapons:
			...
			Button.OnPress (lambda btn=None, idx=i: UseItemDirect (idx))
		Button.SetState (IE_GUI_BUTTON_ENABLED)
		return
...
def FloatMenuSelectNextPC ():
	...
	next_pc = sel % GemRB.GetPartySize () + 1
	GemRB.GameSelectPCSingle (next_pc)
	GemRB.GameSelectPC (next_pc, 1, SELECT_REPLACE)
	# NOTE: it invokes FloatMenuSelectAnotherPC() through selection change handler
	return
