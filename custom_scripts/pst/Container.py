...
# PST override: removes MSGWIN SetVisible toggling that causes
# the dialogue overlay to flash during container open/close.
# Our custom MessageWindow.py manages MSGWIN visibility exclusively
# through UpdateControlStatus() — see GUICommonWindows.py:487,540
# for the same pattern (PST already excluded from MSGWIN toggling there).
...
def OpenContainerWindow ():
	...
	if HideOnClose:
		...
		# PST: skip MSGWIN toggle — already Close()'d by UpdateControlStatus
...
def CloseContainerWindow ():
	...
	ContainerWindow = None
	# PST: skip MSGWIN toggle — UpdateControlStatus manages MSGWIN visibility
